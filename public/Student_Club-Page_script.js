...
function setClubIDInSeeAllLink() {
  ...
  const newUrl = `Student_all-events.html?clubLogo=${encodeURIComponent(clubLogo)}&clubName=${encodeURIComponent(clubName)}`;
...
function setClubIDInSeeAllPost() {
  ...
  // Construct the new URL with the club ID as a query parameter
  const newUrl = `Student_allPosts.html?clubLogo=${encodeURIComponent(clubLogo)}&clubName=${encodeURIComponent(clubName)}`;
