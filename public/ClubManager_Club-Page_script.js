...
function setClubIDInSeeAllLink() {
  ...
  const newUrl = `ClubManager_all-events.html?clubID=${encodeURIComponent(clubID)}&clubName=${encodeURIComponent(clubName)}`;
...
function setClubIDInSeeAllPost() {
  ...
  // Construct the new URL with the club ID as a query parameter
  const newUrl = `ClubManager_allPosts.html?clubID=${encodeURIComponent(clubID)}&clubName=${encodeURIComponent(clubName)}`;
