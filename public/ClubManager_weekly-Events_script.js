          ...
          if (event.eventImage) {
              ...
              imageUrl = event.eventImage.replace(/\\/g, '/');
                      ...
                      card.addEventListener("click", async function() {
                          ...
                          window.location.href = `./ClubManager_Event_details.html?idclubEvent=${encodeURIComponent(event.ideventEvent)}
...
console.log(searchTerm)
      ...
      try {
          // Send an HTTP GET request to the search API endpoint
          const response = await fetch(`/api/Event/search/byname/${searchTerm}`);
          const EventData = await response.json();
          console.log(EventData)
